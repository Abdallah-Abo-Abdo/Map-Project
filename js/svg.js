...
(function () {
  ...
  SVG.extend(SVG.PathArray, {
    ...
    parse: function (array) {
      ...
      SVG.parser.path.setAttribute(
        "d",
        typeof array === "string" ? array : arrayToString(array)
      );
      ...
      for (i = 0, il = segs.numberOfItems; i < il; ++i) {
        ...
        if (
          ...
          if (s == "m")
            segs.replaceItem(
              SVG.parser.path.createSVGPathSegMovetoAbs(x, y),
              i
            );
          else if (s == "l")
            segs.replaceItem(
              SVG.parser.path.createSVGPathSegLinetoAbs(x, y),
              i
            );
          else if (s == "h")
            segs.replaceItem(
              SVG.parser.path.createSVGPathSegLinetoHorizontalAbs(x),
              i
            );
          else if (s == "v")
            segs.replaceItem(
              SVG.parser.path.createSVGPathSegLinetoVerticalAbs(y),
              i
            );
          else if (s == "c")
            segs.replaceItem(
            ...
            segs.replaceItem(
              SVG.parser.path.createSVGPathSegCurvetoQuadraticAbs(x, y, x1, y1),
              i
            );
          else if (s == "t")
            segs.replaceItem(
              SVG.parser.path.createSVGPathSegCurvetoQuadraticSmoothAbs(x, y),
              i
            );
          else if (s == "a")
            segs.replaceItem(
  ...
  SVG.Number = function (value) {
    ...
    if (typeof value === "number") {
      ...
      if (match) {
        ...
        else if (match[2] == "s") this.value *= 1000;
  ...
  SVG.ViewBox = function (element) {
    ...
    while (width.unit == "%") {
      ...
      width = new SVG.Number(
        element instanceof SVG.Doc
          ? element.parent.offsetWidth
          : element.width()
      );
    }
    while (height.unit == "%") {
    ...
    extend: {
      ...
      attr: function (a, v, n) {
        if (a == null) {
          ...
          /* ensure full hex color */ else if (SVG.Color.isColor(v))
            v = new SVG.Color(v);
    ...
    extend: {
      ...
      animate: function (d, ease, delay) {
        ...
        fx.to = function (pos) {
          ...
          if (akeys == null) {
            ...
            if (
              element.morphArray &&
              (fx._plot || akeys.indexOf("points") > -1)
            ) {
              /* get destination */
              var box,
                p = new element.morphArray(
                  fx._plot || fx.attrs.points || element.array
                );
          ...
          if (fx._plot) {
            ...
            if (fx._x) element.x(fx._x.at(pos));
            else if (fx._cx) element.cx(fx._cx.at(pos));

            /* run all y-position properties */
            if (fx._y) element.y(fx._y.at(pos));
            else if (fx._cy) element.cy(fx._cy.at(pos));
          ...
          if (fx._leading) element.leading(fx._leading.at(pos));
            ...
            fx.render = function () {
              if (fx.situation.play === true) {
                ...
                var time = new Date().getTime(),
                  pos =
                    time > fx.situation.finish
                      ? 1
                      : (time - fx.situation.start) / d;
                ...
                if (time > fx.situation.finish) {
                  if (fx._plot)
                    element.plot(
                      new SVG.PointArray(fx._plot.destination).settle()
                    );

                  if (
                    fx._loop === true ||
                    (typeof fx._loop == "number" && fx._loop > 1)
                  ) {
                    if (typeof fx._loop == "number") --fx._loop;
      ...
      stop: function (fulfill) {
        ...
        if (fulfill === true) {
          ...
          if (this._after) this._after.apply(this.target, [this]);
        } else {
  ...
  SVG.registerEvent = function (event) {
    if (!SVG.events[event]) SVG.events[event] = new Event(event);
  };
  ...
  SVG.Doc = SVG.invent({
    ...
    create: function (element) {
      ...
      if (this.parent != this.node) this.stage();
    },
  ...
  SVG.Ellipse = SVG.invent({
    ...
    extend: {
      ...
      width: function (width) {
        return width == null
          ? this.attr("rx") * 2
          : this.attr("rx", new SVG.Number(width).divide(2));
      },
      // Set height of element
      height: function (height) {
        return height == null
          ? this.attr("ry") * 2
          : this.attr("ry", new SVG.Number(height).divide(2));
      },
    ...
    extend: {
      ...
      height: function (height) {
        ...
        return height == null
          ? b.height
          : this.attr(
              this.attr("y1") < this.attr("y2") ? "y2" : "y1",
              b.y + height
            );
      },
  ...
  SVG.extend(SVG.Polyline, SVG.Polygon, {
    ...
    plot: function (p) {
      return this.attr(
        "points",
        (this.array = new SVG.PointArray(p, [[0, 0]]))
      );
    },
  ...
  SVG.Path = SVG.invent({
    ...
    extend: {
      ...
      plot: function (p) {
        return this.attr(
          "d",
          (this.array = new SVG.PathArray(p, [["M", 0, 0]]))
        );
      },
      // Move by left top corner
      move: function (x, y) {
  ...
  SVG.extend(SVG.Text, SVG.TSpan, {
    ...
    tspan: function (text) {
      ...
      if (this instanceof SVG.Text) this.lines.add(tspan);
  ...
  ["fill", "stroke"].forEach(function (m) {
    ...
    extension[m] = function (o) {
      if (
        ...
        this.attr(m, o);
      /* set all attributes from sugar.fill and sugar.stroke list */ else
  ...
  SVG.extend(SVG.Element, {
    ...
    remember: function (k, v) {
      ...
      /* store memory */ else this.memory()[k] = v;
  ...
  function at(o, pos) {
    ...
    return typeof o.from == "number"
      ...
      o instanceof SVG.Color || o instanceof SVG.Number
      ? o.at(pos)
